...
//	private int checkpointProgessCount = 0;
     ...
     */
    private static final Logger logger = Logger.getLogger(ProgressThread.class.getName());
    ...
    public ProgressThread(ProgressStatusProvider statusProvider, Locale locale) {
        ...
        this.resourceBundle = ResourceBundle.getBundle("ProgressResources", locale); //$NON-NLS-1$
    }
    ...
    public void run() {
        if (!System.getProperty("net.jcprogress.disable", "false").equalsIgnoreCase("true")) {
            try {
                ...
                logger.severe("run() - interrupted: " + e.getMessage()); //$NON-NLS-1$
            }
    ...
    public void waitToStop() {
        try {
            ...
            logger.severe("waitToStop() - interrupted exception: " + e.getMessage()); //$NON-NLS-1$
        }
    }
